...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  ...
  //[self.receiver registerForDraggedTypes:[NSArray
  //arrayWithObjects:NSFilenamesPboardType, nil]];
  //[[NSNotificationCenter defaultCenter] addObserver:self
  //selector:@selector(droppedFiles:) name:@"R128X_FilesDropped" object:nil];
  [self.window
      registerForDraggedTypes:[NSArray arrayWithObjects:NSPasteboardTypeFileURL,
                                                        nil]];
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(droppedFiles:)
                                               name:@"R128X_FilesDropped"
                                             object:nil];
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(processedFiles:)
                                               name:@"R128X_FileProcessed"
                                             object:nil];
  [[NSNotificationCenter defaultCenter] addObserver:self
...

{
  ...
  NSArray *fileTypes = [[NSArray alloc]
      initWithObjects:@"mp3", @"mp2", @"m4a", @"wav", @"aif", @"aiff", @"caf",
                      @"alac", @"sd2", @"ac3", nil];
...
- (void)updateView {
  [self.progressIndicator setDoubleValue:0];
  [self.filesInQueueFld
      setStringValue:[NSString
                         stringWithFormat:@"%ld", [self.filesQueue count]]];
}
...
- (void)processNextFileInQueue:(id)launcher {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"R128X_FileProcessed"
                    object:report];
